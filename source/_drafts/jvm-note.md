...
为了能够在方法调用后返回到之前执行的地址，因此在栈帧中，会把返回地址（pc里面的值）放入到调用者栈中进行保存。
...
- 局部变量表。
  ...
  - 局部变量表存放的基本单位是 `slot` (一个32位的大小，64位的需要两个 slot 存储)
...
- stackoverflow。采用固定大小的虚拟机栈，可能分配的局部变量表、操作数栈过多，stackoverflow了
...
- 静态链接。在编译时就已经确定了，对应`早期绑定`（我理解是在 类加载的时候就做了）
- 动态链接。在运行时确定的，对应晚期绑定，实际运行中找到的。

- 为了支持 invokeDynamic 的多态
- 方法在编译后都会将其存入到 静态常量池 中
